...
export const ResultDetailPanel = ({
  ...
  return (
    <div className="card-raised relative animate-fade-in overflow-hidden">
      ...
      <div className="relative space-y-4 p-5">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            ...
            <div className="display mt-1 line-clamp-2 text-xl text-ink-100">{result.title}</div>
          </div>
          <button
            onClick={onToggleFavorite}
            className={cx(
              "shrink-0 rounded-full p-1.5 transition",
              result.favorite ? "text-cream-400 hover:scale-110" : "text-ink-500 hover:scale-110 hover:text-cream-400"
            )}
            aria-label="Favoriye ekle"
            title={result.favorite ? "Favoriden çıkar" : "Favoriye ekle"}
          >
            <Star size={17} strokeWidth={1.5} className={result.favorite ? "fill-cream-400" : ""} />
          </button>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-1.5">
          ...
          {result.resultType === "grid" && (
            <span className="chip inline-flex items-center gap-1">
              <Grid2X2 size={10} /> grid · {result.variations.length}
            </span>
          )}
          ...
          <span className="chip">{formatDate(result.createdAt)}</span>
          {result.favorite && <span className="chip-cream">★ favori</span>}
        </div>

        <div className="space-y-2 rounded-2xl border border-white/[0.05] bg-ink-900/40 p-3">
        ...
        {result.resultType === "grid" && result.variations.length > 0 && (
          <div className="space-y-2 rounded-2xl border border-white/[0.05] bg-ink-900/30 p-3">
            <div className="flex items-center justify-between">
              <div className="inline-flex items-center gap-1.5 text-[10.5px] uppercase tracking-[0.22em] text-cream-400/85">
                <Grid2X2 size={11} /> varyasyonlar
              </div>
              <span className="text-[10px] text-ink-500">{result.variations.length} kare</span>
            </div>
            {result.variationNotes && (
              <p className="rounded-lg bg-ink-950/40 px-2.5 py-1.5 text-[12px] italic leading-relaxed text-ink-300">
                {result.variationNotes}
              </p>
            )}
        ...
        {(character || prompt) && (
          <div className="space-y-2 rounded-2xl border border-white/[0.05] bg-ink-900/30 p-3">
            {character && (
              <Link
                to="/characters"
                className="group flex items-center justify-between gap-2 rounded-lg px-1 py-1 text-sm text-ink-200 transition hover:text-cream-400"
              >
                <span className="inline-flex items-center gap-2">
                  <span className="grid h-6 w-6 place-items-center rounded-full border border-cream-400/30 bg-cream-400/10 text-[10px] font-mono text-cream-400">
        ...
        {result.notes && (
          <div>
            ...
            <p className="whitespace-pre-wrap rounded-2xl border border-white/[0.05] bg-ink-900/40 p-3 text-[13px] leading-relaxed text-ink-300">
              {result.notes}
            </p>
          </div>
        )}
        ...
        {result.issues && (
          <div>
            <div className="label">Hatalar / sorunlar</div>
            <p className="whitespace-pre-wrap rounded-2xl border border-red-500/15 bg-red-950/25 p-3 text-[13px] leading-relaxed text-red-200/85">
              {result.issues}
            </p>
          </div>
        )}
      ...
      <div className="relative grid grid-cols-2 gap-2 border-t border-white/[0.05] bg-ink-900/40 px-5 py-4">
        <button onClick={onEdit} className="btn-primary justify-center">
          <Pencil size={14} /> Düzenle
        </button>
        <button onClick={onExpand} className="btn justify-center">
          <Expand size={14} /> Görseli büyüt
        </button>
        {prompt ? (
          <Link to={`/editor/${prompt.id}`} className="btn justify-center">
            <FileText size={14} /> Promptu aç
          </Link>
        ) : (
          <button disabled className="btn justify-center opacity-40 cursor-not-allowed">
            <FileText size={14} /> Bağlı prompt yok
          </button>
        )}
        <button onClick={onAddSimilar} className="btn justify-center">
          <Copy size={14} /> Benzer ekle
        </button>
        <button
          onClick={onDelete}
          className="btn col-span-2 justify-center text-red-300 hover:!border-red-500/40"
        >
          <Trash2 size={14} /> Sonucu sil
        </button>
      </div>
    </div>
  );
